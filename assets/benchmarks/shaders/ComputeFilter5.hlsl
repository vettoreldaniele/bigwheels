...
[numthreads(1, 1, 1)] void csmain(uint3 tid
    ...
    const float2 samplingDeltas[5][5] = {
        {{-2 * d.x, -2 * d.y}, {-d.x, -2 * d.y}, {0, -2 * d.y}, {d.x, -2 * d.y}, {2 * d.x, -2 * d.y}},
        {{-2 * d.x,     -d.y}, {-d.x,     -d.y}, {0,     -d.y}, {d.x,     -d.y}, {2 * d.x,     -d.y}},
        {{-2 * d.x,      0.0}, {-d.x,        0}, {0,        0}, {d.x,        0}, {2 * d.x,        0}},
        {{-2 * d.x,      d.y}, {-d.x,      d.y}, {0,      d.y}, {d.x,      d.y}, {2 * d.x,      d.y}},
        {{-2 * d.x,  2 * d.y}, {-d.x,  2 * d.y}, {0,  2 * d.y}, {d.x,  2 * d.y}, {2 * d.x,  2 * d.y}},
    };
    // clang-format on

    const float k = 1.0 / 81.0;
