...
float4 map(in float3 pos, float atime)
{
    ...
    float3 cen = float3(0.5 * (-1.0 + 2.0 * t4), pow(p, 2.0 - p) + 0.1, floor(atime) + pow(t1, 0.7) - 1.0);
...
float3 render(in float3 ro, in float3 rd, float time)
{
    ...
    if (res.y > -0.5) {
        ...
        else if (res.y > 1.5) // body
        {
            ...
            col = mix(col, float3(0.14, 0.09, 0.06) * 2.0, (1.0 - res.z) * smoothstep(-0.15, 0.15, -href));
        }
        else // terrain
        {
            // base green
