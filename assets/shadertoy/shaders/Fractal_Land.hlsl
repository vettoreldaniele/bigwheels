...
float2 make_float2(float s) { return float2(s, s); }
float3 make_float3(float s) { return float3(s, s, s); }
...

{
	...
	if (totdist>8.) col=mix(col,max(make_float3(.2),rain.xyz),rain.a*.9);
	if (totdist>8.) col=mix(col,max(make_float3(.2),ncat.xyz),ncat.a*.9);
#endif
#endif
	return col;
}
