...
{
    bool   hasDepthSencil = mDSV.object ? true : false;
    size_t rtvCount       = CountU32(mRTVs);

    // Attachment descriptions
    std::vector<VkAttachmentDescription> attachmentDesc;
    {
        for (uint32_t i = 0; i < rtvCount; ++i) {
            grfx::RenderTargetViewPtr rtv = mRTVs[i].object;
        ...
        if (hasDepthSencil) {
            grfx::DepthStencilViewPtr dsv = mDSV.object;
