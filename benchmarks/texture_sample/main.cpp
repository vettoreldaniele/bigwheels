...
void ProjApp::Setup()
{
    ...
    // Texture image and view
    {
        ...
        for (uint32_t i = 0; i < mNumImages; ++i) {
            grfx::ImagePtr image;
            PPX_CHECKED_CALL(ppxres = grfx_util::CreateImageFromFile(GetDevice()->GetGraphicsQueue(), GetAssetPath("benchmarks/textures/bricks_" + res + ".png"), &image, options, false));
    ...
    // Write descriptors
    {
        ...
        PPX_CHECKED_CALL(ppxres = mDescriptorSet->UpdateDescriptors(CountU32(writes), writes.data()));
        ...
        PPX_CHECKED_CALL(ppxres = mDescriptorSet->UpdateDescriptors(1, &sampler));
    }
}
